#!/usr/bin/env python3
"""Regenerate vertical-slice stub files for hecate-app-ragd.
            ...
            case {slice}_v1:from_map(Params) of
                {{ok, Cmd}} ->
                    case maybe_{slice}:dispatch(Cmd) of
                        ok ->
                            hecate_plugin_api:ok_json(#{{status => accepted}}, Req1);
                        {{error, Reason}} ->
                            hecate_plugin_api:bad_request(reason_to_bin(Reason), Req1)
                    end;
                {{error, Reason}} ->
                    hecate_plugin_api:bad_request(reason_to_bin(Reason), Req1)
            end;
    ...
    case cowboy_req:method(Req0) of
        <<"{http_method.upper()}">> ->
            ...
            case {desk}:handle(Id) of
                {{ok, Result}} ->
                    hecate_plugin_api:ok_json(Result, Req0);
                {{error, not_found}} ->
                    hecate_plugin_api:not_found(Req0);
                {{error, Reason}} ->
                    hecate_plugin_api:bad_request(
    ...
    case cowboy_req:method(Req0) of
        <<"{http_method.upper()}">> ->
            ...
            case {desk}:handle(Params) of
                {{ok, Items}} ->
                    hecate_plugin_api:ok_json(#{{items => Items}}, Req0);
                {{error, Reason}} ->
                    hecate_plugin_api:bad_request(
                        iolist_to_binary(io_lib:format("~p", [Reason])), Req0)
            end;
...
def generate_cmd_apps():
    for app, cfg in CMD_APPS.items():
        deps = ["kernel", "stdlib", "crypto", "cowboy", "reckon_db", "evoq", "reckon_evoq", "hecate_sdk", "rag"]
...
def generate_qry_apps():
    for app, cfg in QRY_APPS.items():
        deps = ["kernel", "stdlib", "crypto", "cowboy", "esqlite", "hecate_sdk", "rag"]
        write(APPS / app / "src" / f"{app}.app.src",
              app_src(app, deps, cfg["description"]))
        write(APPS / app / "src" / f"{app}_app.erl", app_mod(app))
        write(APPS / app / "src" / f"{app}_sup.erl", sup_mod(app))
...
def main():
    print("Scaffolding hecate-app-ragd vertical slices into:")
    print(f"  {APPS}")
    generate_cmd_apps()
    generate_qry_apps()
    generate_prj_apps()
